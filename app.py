...
with col1 :
    with st.container(border=True) :
        st.subheader('D')
        D = st.number_input("D", min_value = 0, max_value = 64, label_visibility = 'hidden')
...
with col2 :
    with st.container(border=True) :
        st.subheader('I')
        I = st.number_input("I", min_value = 0, max_value = 64, label_visibility = 'hidden')
...
with col3 :
    ...
    with st.container(border=True) :
        st.subheader('S')
        S = st.number_input("S", min_value = 0, max_value = 64, label_visibility = 'hidden')
...
with col4 :
    with st.container(border=True) :
        st.subheader('C')
        C = st.number_input("C", min_value = 0, max_value = 64, label_visibility = 'hidden')
